...
*
* // It may be a good idea to encapsulate queue inside its own
class/module and require it with:
* var queue = require('./queue');
*/
var queue = [];

// Tableau des urls visitées et de leurs contenus
var visited = [];

var fs = require('fs');
// Pour vider le fichier :
fs.writeFile('listeUrl.txt', '', function (err) {
  if (err) throw err;
});
...
function get_page(page_url){
  ...
  url:page_url,
    ...
    * The first is an error when applicable (usually from the http.Client
option not the http.ClientRequest object).
    ...
    * -> page size
    * -> language/server behind the web page (php ? apache ? nginx ? using
X-Powered-By)
...
function extract_links(page_url, html_str){
  ...
  (html_str.match(EXTRACT_URL_REG) || []).forEach(function(url){
    ...
    //  On regarde si l'url ne se trouve pas dans la queue ou dans le tableau des urls déjà visitées
    if((queue.indexOf(url)===-1) && (visited.indexOf(url)===-1)){
      if((queue.indexOf('http://'+url)===-1) && (visited.indexOf('http://'+url)===-1)){
        if((queue.indexOf('https://'+url)===-1) && (visited.indexOf('https://'+url)===-1)){
...
app.get('/explorer', function(req, res){
  ...
  queue.forEach(function(url){ 
    request(url, function (error, http_client_response, html_str) { 
      if(error){
        // Si on à une erreur on met quand même l'url dans le tableau des urls visitées pour que l'url ne soit plus scrapé
        visited.push(url);
        queue.shift();
      } else {
        if (http_client_response && (http_client_response.headers['content-type'].indexOf('text/javascript')===-1)) {
          get_page(url);
          visited.push(url); 
          queue.shift();
          console.log("--------> New url visited : "+url);
          fs.appendFile('listeUrl.txt', url+'\n', function (err) {
            if(err){
                console.log(err);
            } else {
...
// Fonctionne avec la page index.html
app.post('/scraping', function(req, res){
  var url = req.body.urlAScraper;
  get_page(url);
  queue.forEach(function(url){ 
    request(url, function (error, http_client_response, html_str) { 
      if(error){
        // Si on à une erreur on met quand même l'url dans les visitées pour que l'url ne soit plus scrapé
        visited.push(url);
        queue.shift();
      } else {
        if (http_client_response && (http_client_response.headers['content-type'].indexOf('text/javascript')===-1)) {
...
app.get('/visited/list', function(req, res){
  res.json(200, {
    visited:{
...
app.get('/visited/size', function(req, res){
  res.setHeader('Content-Type', 'text/plain');
  res.json(200, {
    visited:{
      length:visited.length
